...
app.post('/gareg', (req, res) =>{
    ...
    const message = {
        ...
        ${req.body.elementName}`
    }
    mailer(message)
    user = req.body                                        
    res.redirect('/gareg')                        
    res.send('Спасибо за покупку, заказ будет доставлен в течении двух недель.')
})
app.get('/gareg', (req,res) => {
    if(typeof user !== 'object') return res.sendFile(__dirname + '/Gare.com.html')
    res.send('Спасибо за покупку, заказ будет доставлен в течении двух недель.')
    user = undefined
})
